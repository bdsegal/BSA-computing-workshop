...
This is more of a general tip for reproducibility. If the user doesn't change the folder structure, then we can do this by getting the parent directory,

```{r}
parent <- dirname(getwd())
```

and then using `file.path` to create the file path. Use `file.path` instead of `paste` to write platform indepedent code. 
...
download.file(url="http://www1.ncdc.noaa.gov/pub/data/ghcn/daily/by_year/1887.csv.gz",
  destfile=paste(getwd(),"/1887.csv.gz", sep=""))
```
